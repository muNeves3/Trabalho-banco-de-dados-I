...
function Home({ user, onLogout }: HomeProps) {
  ...
  const [filtrandoMeus, setFiltrandoMeus] = useState(false);
  ...
  const datasetsExibidos = filtrandoMeus
    ? datasets.filter((d) => d.criadorCpf === user.cpf)
    : datasets;
          ...
          <section className="home-section">
            <div className="home-section-header">
              ...
              <p className="home-section-summary">
                {loading ? 'Carregando...' : `${datasetsExibidos.length} dataset${datasetsExibidos.length === 1 ? '' : 's'} disponíveis`}
              </p>
            </div>
